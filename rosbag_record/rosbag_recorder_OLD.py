...
class ROSBagRecorder(Node):
    def __init__(self):
        ...
        #self.create_subscription(Bool, '/terminate_recording', self.terminate_callback, 10)
        
        self.subscribers = {
            # DATA SUBSCRIBERS
            '/obs/image': self.create_subscription(Image, '/obs/image',
                                                        self.img_state_callback, 15),
            '/obs/state': self.create_subscription(Float64MultiArray, '/obs/state',
                                                        self.joint_state_callback, 50),

            '/action': self.create_subscription(Float64MultiArray, '/action', 
                                                self.action_callback, 15),
            '/action_instance': self.create_subscription(String, '/action_instance', 
                                                         self.action_inst_callback, 15),
            '/language_instruction': self.create_subscription(String, '/language_instruction', 
                                                              self.lang_instr_callback, 15),
    ...
    def start_new_episode(self):
        ...
        for i, (topic, msg_type, _) in enumerate([('/obs/image', 'sensor_msgs/msg/Image', Image),
                                                  ...
                                                  ('/action', 'std_msgs/msg/Float64MultiArray', 
                                                               Float64MultiArray),
                                                  ('/action_instance', 'std_msgs/msg/String', String),
                                                  ('/language_instruction','std_msgs/msg/String', 
                                                               String)
                                                 ]
                                       ):
    ...
    def img_state_callback(self, msg):
        self.record_message('/obs/image', msg)

    def joint_state_callback(self, msg):
        jt_state = msg.data
        # Get joint velocities, check if they stopped moving.
        self.jt_vels = jt_state[6:]
        jt_vels_abs = [abs(a) for a in jt_state[6:]]
        jt_vels_np = np.round( np.array(jt_vels_abs), 5)
        self.jts_moving = np.any(jt_vels_np > 0.03)

        self.record_message('/obs/state', msg)
        #print(f"--------> DEBUG: Updated jts_moving = {self.jts_moving}")

        '''self.jt_vels_abs = [abs(a) for a in msg.data[6:]]
        vels = np.round( np.array(self.jt_vels_abs) , 5) 
        if ( np.any(vels > 0.01) and self.recording==True ):
            print("---------------->", vels)'''
        
    def action_callback(self, msg):
        self.action = msg.data
        self.record_message('/action', msg)

    def action_inst_callback(self, msg):
        self.primitive_selected = msg.data
        self.record_message('/action_instance', msg)
    
    def lang_instr_callback(self, msg):
        self.record_message('/language_instruction', msg)
