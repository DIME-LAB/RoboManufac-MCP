...
class VisualServo(Node):
    def __init__(self, topic_name="/object_poses/jenga_4", hover_height=0.15):
        ...
        # Add timer to control update frequency (every 500ms = 2Hz)
        self.update_timer = self.create_timer(0.5, self.timer_callback)
    ...
    def timer_callback(self):
        ...
        if not self.poses_are_similar(position, rpy):
            ...
            self.get_logger().info(f"🔄 Tracking: ({position[0]:.3f}, {position[1]:.3f}) "
                                  f"yaw={rpy[2]:.1f}° (no movement needed)")
